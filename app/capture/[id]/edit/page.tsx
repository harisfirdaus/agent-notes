...
export default async function EditCapturePage({ params }: EditCapturePageProps) {
  ...
  return (
    <AppShell>
      <TopBar title="Edit Capture" />
      <section className="mx-auto max-w-3xl px-5 py-7 lg:px-10">
        <form action={updateCapture.bind(null, id)} className="rounded-2xl border border-border bg-white p-5 shadow-paper sm:p-6">
          <p className="mono-label mb-4 text-ink-muted">Quick Capture</p>
          <textarea
            name="content"
            required
            autoFocus
            defaultValue={capture.content}
